...
// --- Music Player UI Globals ---
volatile bool isMusicStreaming = false;
String currentSong = "";
unsigned long lastMusicStreamTime = 0;
int scrollX = 128;
float cdAngle = 0.0;
...
// HARDCODED DRIVE SYNC PASSWORD
const char* DRIVE_SYNC_PASSWORD = ""; 

// Google Drive API 
const char* DRIVE_CLIENT_ID     ="";
const char* DRIVE_CLIENT_SECRET ="z";
...
// This function encrypts AND decrypts symmetrically in place using the ESP32 hardware AES engine.
void cryptChunk(uint8_t* buffer, size_t len, size_t fileOffset) {
    ...
    size_t nc_off = fileOffset % 16;
    
    memcpy(nonce_counter, AES_IV, 16);
    
    uint32_t block_index = fileOffset / 16;
    uint32_t current_counter = ((uint32_t)nonce_counter[12] << 24) |
                               ((uint32_t)nonce_counter[13] << 16) |
                               ((uint32_t)nonce_counter[14] << 8)  |
                               ((uint32_t)nonce_counter[15]);
                               
    current_counter += block_index;
    
    nonce_counter[12] = (current_counter >> 24) & 0xFF;
    nonce_counter[13] = (current_counter >> 16) & 0xFF;
    nonce_counter[14] = (current_counter >> 8) & 0xFF;
    nonce_counter[15] = current_counter & 0xFF;

    if (nc_off != 0) {
        mbedtls_aes_crypt_ecb(&aes, MBEDTLS_AES_ENCRYPT, nonce_counter, stream_block);
    ...
    mbedtls_aes_crypt_ctr(&aes, len, &nc_off, nonce_counter, stream_block, buffer, buffer);
...
void saveFolderManifest() {
  ...
  if (f) {
    for (String folder : activeFolders) { f.println(folder); }
    f.close();
  }
}
...
void ensureFolders() {
  for (size_t i = 0; i < activeFolders.size(); i++) {
    if (!SD.exists(activeFolders[i])) {
      String path = activeFolders[i]; int idx = 1;
      while (idx > 0) {
        idx = path.indexOf('/', idx);
        String sub = (idx == -1) ? path : path.substring(0, idx);
        if (sub.length() > 0 && !SD.exists(sub)) { SD.mkdir(sub); }
        if (idx != -1) idx++;
      }
    }
  }
...
bool remountSD() {
  ...
  if (ok) {
    loadFolderManifest(); ensureFolders();
...
void appendToManifest(const String& folder, const String& filename) {
  ...
  if (mRead) {
    while (mRead.available()) {
      ...
      if (line == filename) { existsInManifest = true; break; }
    }
    mRead.close();
  }
  if (!existsInManifest) {
    File mWrite = SD.open(manifestPath, FILE_APPEND);
    if (mWrite) { mWrite.println(filename); mWrite.close(); }
  }
}
...
void removeFromManifest(const String& folder, const String& filename) {
  ...
  if (m && t) {
    while (m.available()) {
      ...
      if (line.length() > 0 && line != filename && line != ".manifest.txt") { t.println(line); }
    }
    m.close(); t.close();
    SD.remove(manifestPath); SD.rename(tempPath, manifestPath);
  }
}
...
void renameInManifest(const String& folder, const String& oldName, const String& newName) {
  ...
  if (m && t) {
    while (m.available()) {
      String line = m.readStringUntil('\n'); line.trim();
      if (line.length() > 0) {
        if (line == oldName) { t.println(newName); } else { t.println(line); }
      }
    }
    m.close(); t.close();
    SD.remove(manifestPath); SD.rename(tempPath, manifestPath);
...
String escapeForJsAttr(const String& s) { String out = s; out.replace("\\", "\\\\"); out.replace("'", "\\'"); return out; }
...
String getFileSizeStr(size_t bytes) {
  ...
  if (kb >= 1024.0) return String(kb / 1024.0, 1) + " MB";
  return String((int)kb) + " KB";
}

String formatBytes(uint64_t bytes) {
  if (bytes < 1024) return String((uint32_t)bytes) + " B";
  else if (bytes < (1024 * 1024)) return String((uint32_t)(bytes / 1024.0)) + " KB";
  else if (bytes < (1024 * 1024 * 1024)) return String((uint32_t)(bytes / 1024.0 / 1024.0)) + " MB";
  else return String((uint32_t)(bytes / 1024.0 / 1024.0 / 1024.0)) + " GB";
}
...
String getFolderDisplayName(const String& folderPath) {
  ...
  String name = folderPath; int lastSlash = name.lastIndexOf('/');
  if (lastSlash != -1 && lastSlash < name.length() - 1) { name = name.substring(lastSlash + 1); }
  return name;
}

// ================= HTTP AES RANGE/DECRYPT STREAM HANDLER =================
void serveEncryptedFile(AsyncWebServerRequest *request, String path) {
    if (!SD.exists(path)) {
        request->send(404, "text/plain", "File not found");
        return;
    }

    auto filePtr = std::make_shared<File>(SD.open(path, FILE_READ));
    if (!*filePtr) {
        request->send(500, "text/plain", "Failed to open file for streaming");
        return;
    }

    size_t fileSize = filePtr->size();
    ...
    if (request->hasHeader("Range")) {
        ...
        size_t start = range.substring(0, dashIdx).toInt();
        size_t end = fileSize - 1;
        if (dashIdx != -1 && dashIdx < range.length() - 1) {
            String endStr = range.substring(dashIdx + 1);
            if (endStr.length() > 0) end = endStr.toInt();
        }
        if (start >= fileSize) {
            request->send(416, "text/plain", "Requested Range Not Satisfiable");
            return;
        }
        size_t chunkLen = end - start + 1;
        
        AsyncWebServerResponse *response = request->beginResponse(mime, chunkLen, [filePtr, start](uint8_t *buffer, size_t maxLen, size_t index) -> size_t {
            size_t actualIndex = start + index;
            
            if (filePtr->position() != actualIndex) {
                filePtr->seek(actualIndex);
            }
            
            size_t readBytes = filePtr->read(buffer, maxLen);
            if (readBytes > 0) cryptChunk(buffer, readBytes, actualIndex);
            return readBytes;
        });
        
        response->setCode(206);
        response->addHeader("Content-Range", "bytes " + String(start) + "-" + String(end) + "/" + String(fileSize));
        response->addHeader("Accept-Ranges", "bytes");
        request->send(response);
    } else {
        AsyncWebServerResponse *response = request->beginResponse(mime, fileSize, [filePtr](uint8_t *buffer, size_t maxLen, size_t index) -> size_t {
            if (filePtr->position() != index) {
                filePtr->seek(index);
            }
            
            size_t readBytes = filePtr->read(buffer, maxLen);
            if (readBytes > 0) cryptChunk(buffer, readBytes, index);
            return readBytes;
        });
        response->addHeader("Accept-Ranges", "bytes");
        request->send(response);
...
bool refreshAccessToken() {
  ...
  String body = "client_id=" + String(DRIVE_CLIENT_ID) + "&client_secret=" + String(DRIVE_CLIENT_SECRET) + "&refresh_token=" + String(DRIVE_REFRESH_TOKEN) + "&grant_type=refresh_token";
  yield(); delay(1); int code = http.POST(body); yield(); delay(1);
  String payload = http.getString(); yield(); delay(1); http.end();
...
bool downloadDriveFile(const String& fileId, const String& filename) {
  ...
  http.begin(client, url); http.addHeader("Authorization", "Bearer " + driveAccessToken);
  yield(); delay(1); int code = http.GET(); yield(); delay(1);
  ...
  uint8_t buf[512];
  int total = http.getSize(); int written = 0;
  ...
  while (http.connected() && (total < 0 || written < total)) {
    ...
    if (avail) {
      ...
      cryptChunk(buf, readBytes, written);
...
bool uploadFileToDrive(const String& localPath) {
  ...
  if (!f) return false;
  if (driveAccessToken.length() == 0) { f.close(); return false; }

  size_t fileSize = f.size();
  String filename = localPath;
  int slash = filename.lastIndexOf('/');
  if (slash != -1) filename = filename.substring(slash + 1);
  filename.trim(); filename.replace(" ", "_");

  String sessionUrl;
  {
    WiFiClientSecure client1; client1.setInsecure(); HTTPClient http1;
    ...
    JsonDocument meta; meta["name"] = filename;
    JsonArray parents = meta["parents"].to<JsonArray>(); parents.add(DRIVE_FOLDER_ID);
    String metaBody; serializeJson(meta, metaBody);
    int code1 = http1.POST(metaBody); sessionUrl = http1.header("Location"); http1.end();
    if (code1 != 200 || sessionUrl.length() == 0) { f.close(); return false; }
  }

  yield(); delay(100);
  String urlNoProtocol = sessionUrl; urlNoProtocol.replace("https://", "");
  ...
  if (!client2.connect(host.c_str(), 443)) { f.close(); return false; }
  ...
  while (client2.connected() && !client2.available()) { if (millis() - waitStart > 15000) break; yield(); delay(1); }
...
String syncWithDrive() {
  if (WiFi.status() != WL_CONNECTED) return "No Wi-Fi connection.";
  if (!refreshAccessToken()) return "Failed to get token.";
  WiFiClientSecure client; client.setInsecure(); HTTPClient http;
  String url = "https://www.googleapis.com/drive/v3/files?q=%27" + String(DRIVE_FOLDER_ID) + "%27%20in%20parents%20and%20trashed=false&pageSize=1000&fields=files(id,name,size)";
  http.begin(client, url); http.addHeader("Authorization", "Bearer " + driveAccessToken);
  yield(); delay(1); int code = http.GET(); yield(); delay(1);
  String payload = http.getString(); yield(); delay(1); http.end();
  if (code != 200) return "Failed to list Drive files";

  JsonDocument doc; DeserializationError err = deserializeJson(doc, payload);
  if (err) return "JSON parse failed.";
  JsonArray files = doc["files"].as<JsonArray>();

  int downloaded = 0; int uploaded = 0;
  for (JsonObject file : files) {
    yield(); delay(1);
    String name = file["name"].as<String>();
    String id = file["id"].as<String>();
    String localPath = String(SYNCED_FOLDER) + "/" + name;
    if (!SD.exists(localPath)) { if (downloadDriveFile(id, name)) downloaded++; }
  }
  ...
  if (manifestFile) {
    while (manifestFile.available()) {
      ...
      bool exists = false;
      for (JsonObject file : files) { if (file["name"].as<String>() == name) { exists = true; break; } }
      if (!exists) { String fullPath = String(SYNCED_FOLDER) + "/" + name; if (uploadFileToDrive(fullPath)) uploaded++; }
    }
    manifestFile.close();
  }
  return "Sync complete. Downloaded " + String(downloaded) + ", Uploaded " + String(uploaded);
}
...
void showCentered(String text, int size = 1) {
  ...
  display.println(text); display.display();
}

void showBootScreen() {
  display.clearDisplay(); display.setTextSize(2);
  display.setCursor(12,18); display.println("Media");
  display.setCursor(22,42); display.println("Server");
  display.display();
}

void showWiFiConnecting() {
  display.clearDisplay(); display.setTextSize(1);
  display.setCursor(18,20); display.println("Connecting WiFi");
  display.setCursor(42,40); display.println("..."); display.display();
}

void showIP() {
  display.clearDisplay(); display.setTextColor(SSD1306_WHITE);
  display.setTextSize(2); display.setCursor(0,0); display.println("WiFi");
  display.setTextSize(1); display.setCursor(0,30); display.println(WiFi.localIP());
  display.display(); delay(5000);   
}

void showQRCode(String url) {
  display.clearDisplay(); QRCode qrcode;
  uint8_t qrcodeData[qrcode_getBufferSize(3)];
  qrcode_initText(&qrcode, qrcodeData, 3, ECC_LOW, url.c_str());
  for (uint8_t y = 0; y < qrcode.size; y++) {
    for (uint8_t x = 0; x < qrcode.size; x++) {
      if(qrcode_getModule(&qrcode,x,y)) { display.fillRect(x*2, y*2, 2, 2, SSD1306_WHITE); }
    }
  } display.display(); delay(5000);
}
...
void showDashboard() {
    display.clearDisplay(); display.setTextSize(1);
    display.setCursor(0,0); display.println("ROHAN'S NAS");
    display.drawLine(0,10,128,10,WHITE);
    display.setCursor(0,18); display.print("WiFi : ");
    if(WiFi.status()==WL_CONNECTED) display.println(WIFI_SSID); else display.println("DISCONNECTED");
    display.setCursor(0,30); display.print("IP:"); display.println(WiFi.localIP());
    display.setCursor(0,44); display.print("SD:");
    if(SD.cardType() != CARD_NONE) {
        display.println("Ready");
        uint64_t totalBytes = SD.totalBytes(); uint64_t usedBytes = SD.usedBytes();
        display.setCursor(0, 56); display.print(formatBytes(usedBytes)); display.print(" / "); display.print(formatBytes(totalBytes));
    } else { display.println("Missing"); }
    display.display();
}
...
void updateMusicUI() {
  ...
  display.drawLine(4, 4, 4, 8, SSD1306_WHITE);
  display.drawLine(6, 2, 6, 10, SSD1306_WHITE);
  display.drawLine(8, 0, 8, 12, SSD1306_WHITE);
  
  display.drawRect(110, 2, 14, 8, SSD1306_WHITE);
  display.fillRect(124, 4, 2, 4, SSD1306_WHITE); 
  display.fillRect(112, 4, 10, 4, SSD1306_WHITE); 

  for(int i = 0; i < 16; i++) {
    int h = random(2, 14); 
    display.fillRect(16 + (i*6), 28 - h, 4, h, SSD1306_WHITE);
  }

  display.setTextSize(1);
  int16_t x1, y1; uint16_t w, h_txt;
  display.getTextBounds(currentSong, 0, 0, &x1, &y1, &w, &h_txt);
  display.setCursor(scrollX, 36); display.print(currentSong);
  scrollX -= 3; 
  if (scrollX < -(int)w) { scrollX = 128; }

  int cdX = 20; int cdY = 52; int cdR = 8;
  display.drawCircle(cdX, cdY, cdR, SSD1306_WHITE); 
  display.drawCircle(cdX, cdY, 2, SSD1306_WHITE);   
  int lineX = cdX + (cdR - 1) * cos(cdAngle);
  int lineY = cdY + (cdR - 1) * sin(cdAngle);
  display.drawLine(cdX, cdY, lineX, lineY, SSD1306_WHITE);
  cdAngle += 0.5; 
  if (cdAngle > 6.28) cdAngle = 0; 

  display.fillTriangle(60, 48, 60, 56, 68, 52, SSD1306_WHITE);
  display.drawRect(98, 46, 22, 11, SSD1306_WHITE);
  display.setCursor(101, 48); display.print("MP3");
  display.display();
}
...
String getPageScript() {
  return
    ...
    "function loadMammoth(cb){if(window.mammoth){cb();return;}var s=document.createElement('script');"
      ...
      "s.onload=cb;s.onerror=function(){toastDone('Could not load document viewer',false);};"
    ...
    // --- UPDATED DRIVE SYNC FUNCTIONS WITH PROMPTS ---
    "function runSync(){"
      "var pwd=prompt('Enter Google Drive Sync Password:');"
      "if(!pwd)return;"
      "toastSpinner('Syncing with Drive...');"
      "fetch('/sync?pwd='+encodeURIComponent(pwd),{credentials:'same-origin'})"
      ".then(function(r){if(r.ok)toastDone('Sync started - check Serial Monitor');else toastDone('Invalid Password',false);})"
      ".catch(function(){toastDone('Sync request failed',false);});"
    "}"
    
    "function pushToDrive(path){"
      "var pwd=prompt('Enter Google Drive Sync Password:');"
      "if(!pwd)return false;"
      "toastSpinner('Uploading to Drive...');"
      "fetch('/pushToDrive?path='+encodeURIComponent(path)+'&pwd='+encodeURIComponent(pwd),{credentials:'same-origin'})"
      ".then(function(r){if(r.ok)toastDone('Upload started - check Serial Monitor');else toastDone('Invalid Password',false);})"
      ".catch(function(){toastDone('Upload request failed',false);});return false;"
    "}"
    // --------------------------------------------------

    "function runRemount(){toastSpinner('Remounting SD card...');fetch('/remountSD',{credentials:'same-origin'}).then(function(r){return r.text();}).then(function(txt){var ok=txt.indexOf('successfully')!==-1;toastDone(ok?'SD card remounted':'Remount failed',ok);refreshListings();});}"
    "function deleteFile(path,name){if(!confirm('Delete '+name+'?'))return false;toastSpinner('Deleting '+name+'...');fetch('/deleteFile?path='+encodeURIComponent(path),{credentials:'same-origin'}).then(function(r){return r.text();}).then(function(txt){var ok=txt.indexOf('Deleted')!==-1;toastDone(ok?'Deleted':'Delete failed',ok);refreshListings();});return false;}"
    "function renameFile(path,name){var n=prompt('New name for '+name);if(!n)return false;toastSpinner('Renaming...');fetch('/renameFile?path='+encodeURIComponent(path)+'&newName='+encodeURIComponent(n),{credentials:'same-origin'}).then(function(r){return r.text();}).then(function(txt){var ok=txt.indexOf('Renamed to')!==-1;toastDone(ok?'Renamed':'Rename failed',ok);refreshListings();});return false;}"
    ...
    "function submitMove(){var dest=document.getElementById('moveDest').value;if(!dest)return;closeMoveModal();toastSpinner('Moving...');fetch('/moveFile?path='+encodeURIComponent(currentMovePath)+'&newFolder='+encodeURIComponent(dest),{credentials:'same-origin'}).then(function(r){if(r.ok)return r.text();throw new Error('Failed');}).then(function(txt){toastDone('Moved successfully');refreshListings();});}"
    "function createFolder(ev){ev.preventDefault();var name=document.getElementById('newFolderName').value;if(!name)return false;if(!name.startsWith('/'))name='/'+name;toastSpinner('Creating folder...');fetch('/createFolder?name='+encodeURIComponent(name),{credentials:'same-origin'}).then(function(r){if(r.ok)return r.text();throw new Error('Failed');}).then(function(txt){toastDone('Folder Created');document.getElementById('newFolderName').value='';refreshListings();});return false;}"
    "function deleteFolder(path){if(!confirm('Delete folder '+path+'? Must be empty!'))return false;toastSpinner('Deleting...'); fetch('/deleteFolder?path='+encodeURIComponent(path),{credentials:'same-origin'}).then(function(r){if(r.ok)return r.text();throw new Error('Failed');}).then(function(txt){toastDone('Folder Deleted');refreshListings();});return false;}"
    "function renameFolder(path){var n=prompt('New folder name (e.g. /myfolder):', path);if(!n || n===path)return false;toastSpinner('Renaming...');fetch('/renameFolder?path='+encodeURIComponent(path)+'&newName='+encodeURIComponent(n),{credentials:'same-origin'}).then(function(r){if(r.ok)return r.text();throw new Error('Failed');}).then(function(txt){toastDone('Folder Renamed');refreshListings();});return false;}"
    ...
    "function submitUpload(ev){"
      ...
      "var folderVal=form.querySelector('select[name=\"folder\"]').value;"
      "if(!fileInput.files.length){toastDone('Choose files first',false);return false;}"
      "var files=fileInput.files;var idx=0;"
      "function upNext(){"
        "if(idx>=files.length){toastDone('All uploads complete!');form.reset();refreshListings();return;}"
        "var fd=new FormData();fd.append('folder',folderVal);fd.append('file',files[idx]);"
        "var xhr=new XMLHttpRequest();xhr.open('POST','/upload',true);"
        "xhr.upload.onprogress=function(e){if(e.lengthComputable){var pct=Math.round((e.loaded/e.total)*100);toastSpinner('Uploading '+(idx+1)+'/'+files.length+'... '+pct+'%');}};"
        "xhr.onload=function(){if(xhr.status===200){idx++;upNext();}else{toastDone('Failed: '+files[idx].name,false);}};"
        "xhr.onerror=function(){toastDone('Error: '+files[idx].name,false);};"
        "xhr.send(fd);"
      "}"
      "upNext();return false;"
    "}"
...
String buildFileRowHtml(const String& folderPathStr, const String& name, size_t size) {
  ...
  if (kind != "other") { nameClickAttr = " onclick=\"return openPreview(event,'" + safePath + "','" + kind + "');\""; }
...
String genHomeNext(PageGenState* st) {
  if (st->step == 0) {
    st->step = 1;
    String s = "<html><head><title>Home Media Server</title><meta name='viewport' content='width=device-width, initial-scale=1'>";
    s += getPageStyle(); s += "</head><body><div class='topbar'><h1>&#128187; Home Media Server</h1><div class='actions'>";
    s += "<button class='btn btn-secondary' onclick='runSync()'>&#9729; Sync with Drive</button>";
    s += "<button class='btn btn-secondary' onclick='runRemount()'>&#128260; Remount SD</button>";
    s += "<button class='btn btn-secondary' onclick='refreshListings()'>&#8635; Refresh</button></div></div>";
    s += "<div class='container'><form class='search-form' method='GET' action='/search'><input type='text' name='q' placeholder='Search files across all folders...'><button type='submit'>Search</button></form><div class='grid'>";
    return s;
  }
  if (st->step == 1) {
    if (st->folderIdx >= activeFolders.size()) { st->step = 2; return ""; }
    const String& path = activeFolders[st->folderIdx];
    if (!st->folderOpened) {
      st->manifestFile = SD.open(path + "/.manifest.txt");
      st->folderOpened = true; st->folderAny = false;
      bool prot = (path == "/documents" || path == "/media/audio" || path == "/media/video" || path == "/synced");
      String actions = prot ? "" : "<a class='btn-icon' href='#' onclick=\"return renameFolder('" + escapeForJsAttr(path) + "');\" title='Rename' style='height:24px;width:24px;font-size:0.7em;'>&#9998;</a><a class='btn-icon btn-danger' href='#' onclick=\"return deleteFolder('" + escapeForJsAttr(path) + "');\" title='Delete' style='height:24px;width:24px;font-size:0.7em;'>&#128465;</a>";
      return "<div class='card'><div class='card-header'><h2>" + getFolderDisplayName(path) + "</h2><div style='display:flex;gap:8px;align-items:center;'><span class='card-path'>" + path + "</span>" + actions + "</div></div><div class='file-list'>";
    }
    while (st->manifestFile && st->manifestFile.available()) {
      String name = st->manifestFile.readStringUntil('\n'); name.trim();
      if (name.length() == 0 || name == ".manifest.txt") continue;
      File entry = SD.open(path + "/" + name);
      if (entry && !entry.isDirectory()) {
        st->folderAny = true; size_t fileSize = entry.size(); entry.close();
    ...
    s += "</div></div>"; st->folderOpened = false; st->folderIdx++;
    return s;
  }
  if (st->step == 2) {
    st->step = 3; String s = "</div>";
    String folderOptions = "";
    for (size_t i = 0; i < activeFolders.size(); i++) { folderOptions += "<option value='" + activeFolders[i] + "'>" + getFolderDisplayName(activeFolders[i]) + "</option>"; }
    s += "<div class='panel'><h3>&#128193; Create Folder</h3><form class='upload-row' onsubmit='return createFolder(event);'><input type='text' id='newFolderName' placeholder='/newfolder' required pattern='^/.*'><button type='submit'>Create</button></form></div>";
    
    // --- UPDATED HTML UPLOAD INPUT FOR MULTIPLE FILES ---
    s += "<div class='panel'><h3>&#11014; Upload file(s)</h3><form id='uploadForm' class='upload-row' onsubmit='return submitUpload(event);'><select name='folder'>" + folderOptions + "</select><input type='file' name='file' multiple><button type='submit'>Upload</button></form></div>";
    // ----------------------------------------------------
    
    s += "<div id='moveModalOverlay' onclick=\"if(event.target===this)closeMoveModal();\"><div id='moveModal'><h3>&#128194; Move File</h3><p style='margin:0;color:var(--text-dim);font-size:0.9em;'>Moving: <strong id='moveFileName' style='color:var(--text);'></strong></p><select id='moveDest'>" + folderOptions + "</select><div class='modal-actions'><button type='button' class='btn btn-secondary' onclick='closeMoveModal()'>Cancel</button><button type='button' class='btn' onclick='submitMove()'>Move</button></div></div></div>";
    s += "</div>"; s += getPageScript(); s += "</body></html>"; return s;
  }
  return "";
}
...
void streamSearchResults(AsyncResponseStream* out, const String& query) {
  ...
  int matches = 0;
  for (size_t i = 0; i < activeFolders.size(); i++) {
    File manifestFile = SD.open(activeFolders[i] + "/.manifest.txt");
    ...
    while (manifestFile.available()) {
      ...
      if (nameLower.indexOf(q) != -1) {
        ...
        if (entry && !entry.isDirectory()) {
          String kind = getFileKind(name); String safePath = escapeForJsAttr(fullPath); String safeName = escapeForJsAttr(name);
          String hrefPath = htmlEscape(fullPath); String nameClickAttr = "";
          if (kind != "other") { nameClickAttr = " onclick=\"return openPreview(event,'" + safePath + "','" + kind + "');\""; }
          out->print("<div class='file-row'><a class='file-name' href=\"" + hrefPath + "\"" + nameClickAttr + "><span class='file-icon'>" + getFileIcon(name) + "</span><span class='file-name-text'>" + htmlEscape(fullPath) + "</span></a><span class='file-size'>" + getFileSizeStr(entry.size()) + "</span><span class='file-actions'><a class='btn-icon' href=\"" + hrefPath + "\" download title='Download'>&#11015;</a><a class='btn-icon' href=\"#\" onclick=\"return moveFile('" + safePath + "','" + safeName + "');\" title='Move'>&#128194;</a><a class='btn-icon btn-danger' href=\"#\" onclick=\"return deleteFile('" + safePath + "','" + safeName + "');\" title='Delete'>&#128465;</a></span></div>");
          matches++;
        }
        if (entry) entry.close();
      }
  ...
  if (matches == 0) { out->print("<div class='empty-state'>No files matched.</div>"); }
  out->print("</div></div>");
}
...
void setup() {
  ...
  display.clearDisplay(); showBootScreen(); delay(1500);
  
  SPI.begin(SD_SCK_PIN, SD_MISO_PIN, SD_MOSI_PIN, SD_CS_PIN);
  if (!SD.begin(SD_CS_PIN)) { Serial.println("SD card mount failed! Check wiring."); } 
  else { Serial.println("SD card mounted."); loadFolderManifest(); ensureFolders(); }
  ...
  showIP(); showQRCode(ngrokURL); showDashboard();
      ...
      [state](uint8_t *buffer, size_t maxLen, size_t /*index*/) -> size_t {
        ...
        while (written < maxLen) {
          if (state->pending.length() == 0) {
            if (state->step >= 3) break;
            state->pending = genHomeNext(state.get());
            if (state->pending.length() == 0 && state->step < 3) continue;
          }
          ...
          written += toCopy; state->pending.remove(0, toCopy);
        }
        return written;
      });
    request->send(response);
  ...
  server.on("/mediaStream", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (!keyOk && !request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("path")) { request->send(400, "text/plain", "Missing path."); return; }
    String path = request->getParam("path")->value();
    if (!SD.exists(path)) { request->send(404, "text/plain", "Not found."); return; }

    String mime = getMimeForPath(path);
    if (mime.startsWith("audio/")) {
        int slash = path.lastIndexOf('/'); currentSong = (slash != -1) ? path.substring(slash + 1) : path;
        isMusicStreaming = true; lastMusicStreamTime = millis(); scrollX = 128; 
    }
    
    serveEncryptedFile(request, path); 
  });

  server.onNotFound([](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    String path = request->url();
    if (SD.exists(path) && !SD.open(path).isDirectory()) { serveEncryptedFile(request, path); } 
  ...
  server.on("/remountSD", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    bool ok = remountSD();
    request->send(200, "text/html", ok ? "<p>Remounted successfully.</p><a href='/'>Go back</a>" : "<p>Remount failed.</p>");
  });

  server.on("/sync", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("pwd") || request->getParam("pwd")->value() != String(DRIVE_SYNC_PASSWORD)) {
        request->send(403, "text/plain", "Forbidden: Invalid Sync Password");
        return;
    }
    syncRequested = true;
    request->send(200, "text/html", "<h3>Sync Started</h3><p>Check Serial Monitor.</p><a href='/'>Go back</a>");
  });
  ...
  server.on("/pushToDrive", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (!request->hasParam("path")) { request->send(400, "text/html", "<p>Missing file.</p>"); return; }
    pushPath = request->getParam("path")->value();
    pushRequested = true;
    request->send(200, "text/html", "<h3>Upload Started</h3>");
  });

  server.on("/moveFile", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("path") || !request->hasParam("newFolder")) { request->send(400, "text/plain", "Error"); return; }
    String path = request->getParam("path")->value(); String newFolder = request->getParam("newFolder")->value();
    ...
    if (oldFolder == newFolder) { request->send(400, "text/plain", "Already in folder"); return; }
    if (!SD.exists(newFolder)) { request->send(400, "text/plain", "Folder missing"); return; }
    if (SD.rename(path, newPath)) {
        removeFromManifest(oldFolder, filename); appendToManifest(newFolder, filename);
        showStatus("Moved File\n" + filename); delay(1000); showDashboard();
        request->send(200, "text/plain", "Moved");
    } else { request->send(500, "text/plain", "Move failed."); }
  });

  server.on("/createFolder", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("name")) { request->send(400, "text/plain", "Error"); return; }
    String name = request->getParam("name")->value(); if (!name.startsWith("/")) name = "/" + name;
    if (SD.exists(name)) { request->send(400, "text/plain", "Exists"); return; }
    if (SD.mkdir(name)) {
      activeFolders.push_back(name); saveFolderManifest();
      showStatus("Folder Created\n" + name); delay(1000); showDashboard();
  ...
  server.on("/deleteFolder", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("path")) { request->send(400, "text/plain", "Error"); return; }
    String path = request->getParam("path")->value();
    if (path == "/" || path == "/documents" || path == "/media/audio" || path == "/media/video" || path == "/synced") { request->send(400, "text/plain", "System folder"); return; }
    bool canDelete = true; String manifestPath = path + "/.manifest.txt";
    File m = SD.open(manifestPath);
    if (m) { while (m.available()) { String line = m.readStringUntil('\n'); line.trim(); if (line.length() > 0 && line != ".manifest.txt") { canDelete = false; break; } } m.close(); }
    if (!canDelete) { request->send(400, "text/plain", "Not empty"); return; }
    if (SD.exists(manifestPath)) SD.remove(manifestPath);
    if (SD.rmdir(path)) {
      for (auto it = activeFolders.begin(); it != activeFolders.end(); ++it) { if (*it == path) { activeFolders.erase(it); break; } }
      saveFolderManifest(); showStatus("Folder Deleted\n" + path); delay(1000); showDashboard();
  ...
  server.on("/renameFolder", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("path") || !request->hasParam("newName")) { request->send(400, "text/plain", "Error"); return; }
    String path = request->getParam("path")->value(); String newName = request->getParam("newName")->value();
    if (!newName.startsWith("/")) newName = "/" + newName;
    if (path == "/" || path == "/documents" || path == "/media/audio" || path == "/media/video" || path == "/synced") { request->send(400, "text/plain", "System folder"); return; }
    if (SD.rename(path, newName)) {
      for (size_t i = 0; i < activeFolders.size(); i++) { if (activeFolders[i] == path) { activeFolders[i] = newName; break; } }
      saveFolderManifest(); showStatus("Folder Renamed\n" + newName); delay(1000); showDashboard();
  ...
  server.on("/deleteFile", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("path")) { request->send(400, "text/html", "Error"); return; }
    String path = request->getParam("path")->value(); int slash = path.lastIndexOf('/');
    String folder = (slash != -1) ? path.substring(0, slash) : ""; String filename = (slash != -1) ? path.substring(slash + 1) : path;
    if (SD.remove(path)) { removeFromManifest(folder, filename); showStatus("File Deleted\n" + filename); delay(1000); showDashboard(); request->send(200, "text/html", "Deleted"); } 
    else { request->send(200, "text/html", "Failed"); }
  });

  server.on("/renameFile", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
    if (!request->hasParam("path") || !request->hasParam("newName")) { request->send(400, "text/html", "Error"); return; }
    String path = request->getParam("path")->value(); String newName = request->getParam("newName")->value();
    int slash = path.lastIndexOf('/'); String folder = (slash != -1) ? path.substring(0, slash) : "";
    String oldName = (slash != -1) ? path.substring(slash + 1) : path; String newPath = folder + "/" + newName;
    if (SD.rename(path, newPath)) { renameInManifest(folder, oldName, newName); showStatus("File Renamed\n" + newName); delay(1000); showDashboard(); request->send(200, "text/html", "Renamed"); } 
    else { request->send(200, "text/html", "Failed"); }
  });
  ...
  server.on("/search", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    response->print("<html><head><title>Search</title><meta name='viewport' content='width=device-width, initial-scale=1'>");
    response->print(getPageStyle()); response->print("</head><body><div class='topbar'><h1>&#128269; Search</h1><div class='actions'><a class='btn btn-secondary' href='/'>&larr; Back to files</a></div></div><div class='container'>");
    streamSearchResults(response, q); response->print("</div>"); response->print(getPageScript()); response->print("</body></html>");
    request->send(response);
  });
  ...
  server.on("/upload", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return request->requestAuthentication();
      request->send(200, "text/html", "<p>Upload complete.</p>");
    },
    [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
      if (!request->authenticate(AUTH_USERNAME, AUTH_PASSWORD)) return;
      static File uploadFile; static String targetFolder = "/documents";
      if (index == 0) {
        showStatus("Uploading...\n" + filename);
        if (request->hasParam("folder", true)) targetFolder = request->getParam("folder", true)->value();
        String path = targetFolder + "/" + filename; SD.remove(path);
        uploadFile = SD.open(path, FILE_WRITE);
      }
      if (uploadFile) {
        cryptChunk(data, len, index);
        uploadFile.write(data, len);
      }
      if (final) {
        if (uploadFile) uploadFile.close();
        appendToManifest(targetFolder, filename);
        showStatus("Upload Complete\n" + filename); delay(1500); showDashboard();
      }
    });
...
void loop() {
    if (syncRequested) {
        ...
        delay(1500); showDashboard();
        Serial.println("Sync Log: " + lastSyncResult);
    }

    if (pushRequested) {
        pushRequested = false;
        if (WiFi.status() != WL_CONNECTED) { lastPushResult = "No Wi-Fi"; } 
        else if (!refreshAccessToken()) { lastPushResult = "Token Failed"; } 
        else {
            showStatus("Uploading...");
            if (uploadFileToDrive(pushPath)) { lastPushResult = "Upload Done"; showStatus("Upload Done"); delay(1500); } 
            else { lastPushResult = "Upload Failed"; showStatus("Upload Failed"); delay(1500); }
            showDashboard();
        }
        Serial.println("Push Log: " + lastPushResult);
    }

    if (isMusicStreaming) {
        if (millis() - lastMusicStreamTime > 60000) { 
            isMusicStreaming = false; showDashboard();
        } else { updateMusicUI(); }
    }
